...
def get_block(grid: List[List[str]], pos: Tuple[int, int]) -> List[str]:
    ...
    c = (c//3)*3
    block_values = []
    for row_ind in range(r, r+3):
        for col_ind in range(c, c+3):
            block_values.append(grid[row_ind][col_ind])
    return block_values
...
def find_possible_values(grid: List[List[str]], pos: Tuple[int, int]) -> Set[str]:
    ...
    return {'1', '2', '3', '4', '5', '6', '7', '8', '9'}-values
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for col_ind in range(9):
        col = get_col(solution, (0, col_ind))
...
def generate_sudoku(N: int) -> List[List[str]]:
    ...
    while show > 0:
        random_row = random.randrange(9)
        random_col = random.randrange(9)
